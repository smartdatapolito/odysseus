...
def load_plot_df(bookings_by_hour, agg_freq_, count_col):

    plot_df = bookings_by_hour[["datetime", count_col]].set_index("datetime").resample(
        agg_freq_
    ).sum().asfreq(agg_freq_, fill_value=0).rename(columns={count_col: "bookings_count"})

    return plot_df


def get_bookings_count_plot_data(city):
    ...
    bookings_by_hour = load_bookings_by_hour(city, data_source_id,year,month) #citta,sorgente,anno,mese  
    ...
    return bookings_by_hour#, bookings_count, deleted_bookings_count, bubble_plot_df
